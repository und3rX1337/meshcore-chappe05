...
if [[ $1 == "build-firmware" ]]; then
  ...
  # STM32 repeater targets excluded from CI: RadioLib fails to build against the recent STM32 Arduino core (PinMode/PinStatus typing); none of these boards are deployed on our network
  get_pio_envs_ending_with_string "_repeater" | grep -vE '^(wio-e5-mini|wio-e5|RAK_3x72|Tiny_Relay)_repeater$'
